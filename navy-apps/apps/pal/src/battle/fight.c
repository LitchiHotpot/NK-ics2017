...
--*/
{
   ...
   if (wAttackStrength > wDefense)
   {
	 ...
	 for(int i=0;i<1;i++){
      ...
      sDamage = F2int(int2F(wAttackStrength << 1) - F_mul_int(s1, wDefense) + s2);
   }
   else if (wAttackStrength > F2int(F_mul_int(total, wDefense)))
   {
	 FLOAT s2 = int2F(5);
	 for(int i=0;i<1;i++){
	   s2=F_div_int(s2,10);
	 }
      sDamage = F2int(int2F(wAttackStrength) - F_mul_int(total, wDefense) + s2);
   }
   else
   {
      sDamage = 0;
...
--*/
{
   ...
   else
   {
	 ...
	 for(int i=0;i<1;i++){
      ...
      g_Battle.flTimeChargingUnit = F_div_F(g_Battle.flTimeChargingUnit, s4);
   }
}
	   ...
	   a2=F_div_int(a2,10);
            ...
            for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++)
            {
               ...
               else
               {
				  ...
				  wDexterity = F2int(F_mul_int(RandomFloat(b1,b2),
							  wDexterity));
...
--*/
{
   ...
   case kBattleActionAttack:
      ...
      if (sTarget != -1)
      {
				    FLOAT total2 = int2F(1125);
					for(int i=1;i<4;i++){
					  total2=F_div_int(total2,10);
					}
         //
         // Attack one enemy
         //
         for (t = 0; t < (gpGlobals->rgPlayerStatus[wPlayerRole][kStatusDualAttack] ? 2 : 1); t++)
         {
            ...
            sDamage = F2int(F_mul_int(RandomFloat(int2F(1), total2), sDamage));
